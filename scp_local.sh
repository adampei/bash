...
while getopts p:h flag; do
    ...
    p) FILE_PATH=${OPTARG} ;;
    h)
        echo -e "\033[32mUsage: $0 -p /path/on/server/to/your/file\033[0m"
        exit 0
        ;;
    esac
done
...
# 检查是否已经输入了文件路径
if [ -z "$FILE_PATH" ]; then
    echo -e "\033[31mError: Please specify a file path on the server with the -p option.\033[0m"
    echo -e "\033[32mUsage: $0 -p /path/on/server/to/your/file\033[0m"
    exit 1
fi
...
# 使用scp命令将文件从远程服务器复制到本地的当前目录
scp root@$SERVER_IP:$FILE_PATH ./